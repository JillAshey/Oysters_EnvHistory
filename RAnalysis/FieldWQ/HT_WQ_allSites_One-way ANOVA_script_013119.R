...
##for spec_pH
fit_spec <-lm(spec_pH~Site, na.action = na.exclude, data=p)
summary(fit_spec)
require(car)
...
##for salinity
fit_sal <-lm(Salinity~Site, na.action = na.exclude, data=p)
summary(fit_sal)
require(car)
...
##for DOmg
fit_DOm <-lm(DO_mg~Site, na.action = na.exclude, data=p)
summary(fit_DOm)
require(car)
...
##for TA
fit_TA <-lm(TA~Site, na.action = na.exclude, data=p)
summary(fit_TA)
require(car)
