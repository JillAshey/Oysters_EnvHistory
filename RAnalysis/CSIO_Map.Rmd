...
# Read in data 
```{r}
CSIO_loc <- read_excel("data/SiteMap/CSIO_Locations.xlsx")
...
ggsave("output/CSIO_CB_plot.pdf", plot = CB_plot, width = 10, height = 8)

#CB_subset = subset(CB, xmin =)

# Subset the data set for just the VA portion of the Bay
VA_CB <- subset(CB, STATE == "Virginia" | STATE == "Maryland")
plot(VA_CB)
```
...
ggplot() + geom_polygon(data = VA_CB_crop, aes(x=long,y=lat, group=group), fill="gray", colour="black") +
  coord_equal() +
  geom_point(data = CSIO_loc, aes(x=Long, y=Lat, color = Status), size = 3) +
  scale_color_manual(values = c("green", "black")) +
  geom_label_repel(data = CSIO_loc, aes(x=Long, y=Lat, label = CSIO_loc$SiteName), size = 4, direction = "both", nudge_y = 0.015, segment.alpha = 0) +
...
# Save plot
ggsave("output/CSIO_Locations.pdf", device = "pdf", width = 12, height = 12, dpi = 600)
```
