...
##Set 1
#reading in data
dat1 = read.csv(file="DistalSites_PhysWQ_HT_030220.csv", header=TRUE) #file is now likely named HT_PhysWQ_Set1_051620
View(dat1)
...
setwd("~/Desktop/R/HT/csv_files/Physiology_WQ")

#reading in data
dat2 <- read_csv("HT_PhysWQ_Set2_051620.csv")
View(dat2)
...
FRAPtest = read.csv(file="~/Desktop/R/HT/csv_files/FRAP/HT_FRAP_Siteavgs_043020.csv", header=TRUE)
View(FRAPtest)
...
p3 = ggplot(data = FRAPtest, aes(x=Site, y=dFRAPavg,
  ...
  theme_minimal() +
  theme(axis.line.y = element_line(), axis.text.x = element_text(size = 12, angle=30, hjust = 1, colour = "black"), axis.text.y = element_text(size=12, color = "black"), axis.title.y = element_text(size = 15))
#Ugly plots but will beautify and arrange them below!
