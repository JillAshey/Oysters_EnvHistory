...
##FRAP - set 1
antiox1 = read.csv(file="FRAP_HT_Set1.csv", header=TRUE)
View(antiox1)
                          

FRAPtest = read.csv(file="FRAP_test.csv", header=TRUE)
View(FRAPtest)


##FRAP - set 1
ggplot(data = FRAPtest, aes(x=Site, y=FRAPavg1, fill=Treatment, 
                            ymin = FRAPavg1 - FRAPse1, ymax = FRAPavg1 + FRAPse1)) +
...
##FRAP - set 2
ggplot(data = FRAPtest, aes(x=Site, y=FRAPavg2, fill=Treatment, 
                            ymin = FRAPavg2 - FRAPse2, ymax = FRAPavg2 + FRAPse2)) +
  geom_bar(stat = "identity", position= "dodge", width = 0.75) +
  labs(y="FRAP", x="")+ 
  geom_errorbar(width = 0.2, position = position_dodge(0.75), size = 0.5) +
  scale_y_continuous(expand = c(0,0)) + 
...
##delta FRAP
ggplot(data = FRAPtest, aes(x=Site, y=dFRAPavg, fill=Treatment, 
