#Script for running one-way ANOVA --initial shell height 
...
library(lme4)
inital = read.csv("InitialShellHeight_HT_022219.csv", head = T)
...
fit2 <-lm(Growth_Rate~Site, na.action = na.exclude, data=grow)
...
##for gly distal % of dry weight--after arcsine transformation
...
##change in gly content for % change--after arcsine transformation
...
#for gly change after +0.1 and transformation
fit2 <-lm(P_change_transform~Site, na.action = na.exclude, data=gly)
summary(fit2)
require(car)
Anova(fit2, type="III")
test <- glht(fit2, linfct = mcp(Site = "Tukey"))
