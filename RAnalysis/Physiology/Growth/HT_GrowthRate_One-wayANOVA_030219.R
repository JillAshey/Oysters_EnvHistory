#Script for running one-way ANOVA --growth rate (mm/day)
#03/02/19
...
library(lme4)
trial = read.csv("Growth_HT_results_021719.csv", head = T)
...
fit2 <-lm(GrowthRate_mm_day~Site, na.action = na.exclude, data=trial)
...
#the final shell heights are different, the initial shell heights the same--
#so shouldn't the growth rates differ?
