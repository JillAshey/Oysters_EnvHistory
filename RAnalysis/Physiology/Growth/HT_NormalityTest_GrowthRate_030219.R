##Analysis of growth rate (mm/day) for homogeneity 
setwd("~/Desktop")
trial = read.csv("Growth_HT_results_021719.csv", header=T)
blah = read.csv("ConditionIndex_HT.csv", header=T)
head(trial)
library(car)

CI_proximal = blah[1:34,]
CI_distal = blah[35:74,]
...
leveneTest(GrowthRate_mm_day~Site, data=trial) #normal
leveneTest(Change_T~Site, data=gly_distal) #normal
leveneTest(Prop_Gly~Site, data=gly_distal) #normal
leveneTest(Prop_Gly~Site, data=gly_proximal) #normal

leveneTest(Prop_CI~Site, data=CI_proximal) #normal
leveneTest(Prop_CI~Site, data=CI_distal) #normal
leveneTest(Prop_DeltaCI~Site, data=CI_distal) #normal 
...
###p>0.05 means the variances are equal
bartlett.test(GrowthRate_mm_day~Site, data=trial) #normal
bartlett.test(Change_T~Site, data=gly_distal) #not normal???
bartlett.test(Prop_Gly~Site, data=gly_distal) #not normal
bartlett.test(Prop_Gly~Site, data=gly_proximal) #normal
...
fit<-lm(Change_T~Site, data=gly_distal)
par(mfrow=c(2,2))
